...
function predictNextPurchase($userID, $productID)
{
	...
	$result = mysqli_fetch_assoc(mysqli_query($_SESSION["dbConnection"], "SELECT id FROM events WHERE user = $userID AND product = $productID"));
	if($result["id"] != null)
	{
		$eventID = $result["id"];
	...
	mysqli_select_db($_SESSION["dbConnection"],"ShopBuddy");
	$result = mysqli_query($_SESSION["dbConnection"], "SELECT * FROM Scans WHERE productID = $productID ORDER BY time DESC");
	...
	if(mysqli_num_rows($result) > 1)
	{
		...
		$url = $productInfo['url'];
		$title =mysqli_real_escape_string($_SESSION["dbConnection"], $productInfo['name']);
...
function sendTextMessage($userID, $productID)
{
	mysqli_select_db($_SESSION["dbConnection"], "ShopBuddy");
